...
unsigned char send_LCP_REQ(struct Connection_info *my_connection)
{
    ...
    struct pppoe_packet ppp_packet;
    memset(&ppp_packet,0,sizeof(ppp_packet));

    m_num.Type=0x05;
    m_num.Length=0x06;
    m_num.Data=htons((int)(rand()/(float)RAND_MAX*2048));
    my_connection->LCP_magic_number=m_num.Data; //sure the magic number

    mru.Type=0x01;
    mru.Length=0x04;
    mru.Data=htons(1492);    //sure the mru
    ...
    memcpy(ppp_frame.Information,&lcp_packet,sizeof(lcp_packet)); //one by one copy

    memcpy(ppp_packet.eth_dst_mac,my_connection->peer_mac,MAC_LEN);
    memcpy(ppp_packet.eth_src_mac,my_connection->my_mac,MAC_LEN);
    ppp_packet.eth_type=htons(ETHER_TYPE_PPP_SESSION);
    ppp_packet.pppoe_type=0x01;
    ppp_packet.pppoe_ver=0x01;
    ppp_packet.pppoe_code=CODE_OF_PPP_SESSION;
    ppp_packet.pppoe_session_id=my_connection->pppoe_session_id;
    ppp_packet.pppoe_length=htons(ntohs(lcp_packet.Length)+2);
    memcpy(ppp_packet.payload,&ppp_frame,sizeof(lcp_packet.Length)+2);

    if(send(session_socket,&ppp_packet,ETH_HEARER_LEN_WITHOUT_CRC+PPPOE_HEADER_LEN+ntohs(pppoe_packet.pppoe_length),0))
    {
        printf("error in LCP_REQ!\n%s\n",strerror(errno));
        exit(1);
    }
    else
    {
...
int LCP_handle(struct Connection_info *my_connection,struct pppoe_packet *ppp_packet,unsigned char LCP_ID)
{
    struct ppp_frame_for_pppoe *ppp_frame=(struct ppp_frame_for_pppoe *)ppp_packet->payload;
    ...
    if(lcp_packet->Code==0x09)
    {
        ...
        unsigned char temp[MAC_LEN];  //exchange the src and dst
        memcpy(temp,ppp_packet->eth_src_mac,MAC_LEN);
        memcpy(ppp_packet->eth_src_mac,ppp_packet->eth_dst_mac,MAC_LEN);
        memcpy(ppp_packet->eth_dst_mac,temp,MAC_LEN);

        ppp_packet->pppoe_length=htons(10);
    ...
    if(lcp_packet->Code==0x01)
    {
        ...
        struct pppoe_packet req_ack;
        memcpy(req_ack.eth_dst_mac,my_connection->peer_mac,MAC_LEN);
        memcpy(req_ack.eth_src_mac,my_connection->my_mac,MAC_LEN);
        req_ack.eth_type=htons(ETHER_TYPE_PPP_SESSION);
        req_ack.pppoe_type=0x01;
        req_ack.pppoe_ver=0x01;
        req_ack.pppoe_code=0x00;
        req_ack.pppoe_session_id=my_connection->pppoe_session_id;
        req_ack.pppoe_length=htons(ntohs(lcp_packet->Length)+2);
        struct ppp_frame_for_pppoe *ppp_frame_req_ack=(struct ppp_frame_for_pppoe *)req_ack.payload;
        ppp_frame_req_ack->Protocol_Field=htons(LCP);
        memcpy(ppp_frame_req_ack.Information,lcp_packet,ntohs(lcp_packet->Length));

        int sent_count=send(session_socket,&req_ack,ETH_HEARER_LEN_WITHOUT_CRC+PPPOE_HEADER_LEN+ntohs(req_ack.pppoe_length),0);
        if(sent_count==-1) {
